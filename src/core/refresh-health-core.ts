...
export interface RefreshHealthFields {
...
): RefreshHealthFields['recentRuns'] {
  return entries
    .filter((e) => !inFlight(e))
    .map((e) => {
      const ok = successLike(e.status);
      const run: NonNullable<RefreshHealthFields['recentRuns']>[number] = { ok, endTime: e.endTime };
...
> {
  ...
  let lastStatus: RefreshHealthFields['lastStatus'];
...
): Pick<RefreshHealthFields, 'scheduleSummary' | 'scheduleOverdue'> {
...
): Pick<RefreshHealthFields, 'lastStatus' | 'lastAttemptTime' | 'lastSuccessTime' | 'recentRuns'> {
  if (entries.length === 0) return { lastStatus: 'Never' };
